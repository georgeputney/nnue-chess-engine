"""The NNUE network as a torch module - training only. engine/net.py is the numpy twin the
...
Shape (see engine/arch.py for the feature set):
...
piece count selects (engine.arch.output_bucket). The forward here takes the two perspective planes
...
from engine.arch import FEATURES, FT_OUT, L1_OUT, L2_OUT, OUTPUT_BUCKETS
...
class NNUE(nn.Module):
    # ft_out is the accumulator width per colour. The default matches engine/arch.py; a narrower
